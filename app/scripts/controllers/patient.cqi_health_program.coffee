...
angular.module 'bcd'
  .controller 'ctrl.patient.CqiHealthProgram', ($rootScope, $scope, $state, $filter, $q, $timeout, Treatments, TreatmentDecorator, treatmentTemplates, sidebar, sidebarActiveItem, pageCaption, Notification) ->
    $scope.items = treatmentTemplates
    $scope.sidebar = 'patient.request_treatment'
    $scope.sidebarActiveItem = sidebarActiveItem
    $rootScope.pageCaption = pageCaption
    $scope.form = false
    $scope.stage = 1
    $rootScope.header = 'patient.request_treatment'
    ...
    $scope.toggleSelect = (item) ->
      ...
      $scope.options.showThanks = false
      $state.go $scope.states().list if $scope.selectedItemsCount() is 0 and $rootScope.currentState.state.name is $scope.states().selected
    ...
    $scope.toForm = ->
      ...
      $scope.selectedItems.forEach (item) ->
        ...
        item.receiver_id = $scope.currentUser.id
        item.full_name = (html) ->
    ...
    $scope.backToList = ->
      ...
      $state.go 'patient.cqi_treatments'
    ...
    $scope.submit = mutexAction $scope, ->
      ...
      .success (data) ->
        ...
        if count_of_selected_items is 1
          Notification.success("#{name} was requested")
        else
          Notification.success
            title: "Requested #{count_of_selected_items} treatments",
